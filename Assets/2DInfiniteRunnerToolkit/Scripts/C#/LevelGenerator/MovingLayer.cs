...
public class MovingLayer : MonoBehaviour 
{
	...
	void Update () 
    {
        if (!paused)
        {
            ...
            foreach (Transform item in activeElements)
            {
                ...
                item.transform.position -= Vector3.up * startingSpeed * speedMultiplier * Time.deltaTime;
                
                //If the item has reached the reset position
                if (item.transform.position.y < resetAt)
                    removeLast = true;
            }
    ...
    private void RemoveElement(Transform item)
    {
        ...
        //Reset it's position
        item.transform.position = new Vector3(item.transform.position.x, startAt, 0);
    }
