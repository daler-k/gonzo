...
public class PlayerManager : MonoBehaviour
{
    ...
    private Vector3 startingPosCam; 
    ...
    public Animator pixelRedAnimator;
    public GameObject pixelRedGO;

    public GameObject blood;
    public GameObject gonzo;

    public bool inputWithTouch;
    ...
    void Start()
    {
        ...
        startingPosCam = _camera.transform.position;
    ...
    public void setCrashed(bool crashVal)
    {
        crashed = crashVal;
    }
    ...
    void Update()
    {
        ...
        {
            if (health <= 0 && playerStatus != PlayerStatus.Sinking)
            {
            ...
            if (!inputWithTouch)
            {
                if (Input.GetAxis("Horizontal") > 0f || Input.acceleration.x > 0f)
                {
                    //Debug.Log("up" + ":" + Input.GetAxis("Vertical"));
                    UpdateInput(true);
                }
                else if (Input.GetAxis("Horizontal") < 0f || Input.acceleration.x < 0f)
                {
                    //Debug.Log("down" + ":" + Input.GetAxis("Vertical"));
                    UpdateInput(false);
                }
            }
            else
            ...
            else if (playerStatus == PlayerStatus.Sinking && !crashed)
            {
                Sink();
                crashed = true;
    ...
    void Fire()
    {
        ...
        iTween.PunchPosition(gun, iTween.Hash("y", .5f, "time", .3f));
       // iTween.PunchPosition(ping, iTween.Hash("y", .5f, "time", .3f));
        //Instantiate(shot, shotSpawn.position - new Vector3(.3f, 0f, 0f), shotSpawn.rotation);
        //Instantiate(shot, shotSpawn.position + new Vector3(.3f, 0f, 0f), shotSpawn.rotation);
        //audio.Play();
    }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        else if ((other.tag == "Enemy" || other.tag == "Hole") && playerVulnerability != PlayerVulnerability.Shielded && !powerupManager.isExtraSpeedActive)
        {
            if (other.tag == "Hole")
                audioManager.PlayGreenHole();
            else
                audioManager.PlayHeroHurt();

            pixelRedGO.SetActive(true);
            pixelRedAnimator.Play("open");
            //if (health >= 1)
                health -= 1;
            //else
            //    playerStatus = PlayerStatus.Sinking;
            guiManager.UpdateHealth(health);
        }
        else if (other.tag == "Enemy" && playerVulnerability == PlayerVulnerability.Shielded) //knife
        {
            AudioManager.Instance.PlayZomboDied();
            Destroy(other.gameObject);
            Instantiate(blood, other.gameObject.transform.position, other.gameObject.transform.rotation);

           // CollapseShield();
        }
        else if (powerupManager.isExtraSpeedActive) //ExtraSpeed
        {
            //Destroy(other.gameObject);
            Destroy(Instantiate(blood, other.gameObject.transform.position, other.gameObject.transform.rotation),1f);
        }
	...
	public void Reset()
    {
        ...
        _camera.transform.position = startingPosCam;
	...
	public void UpdateInput(bool inputActive)
    {
        ...
        if (playerStatus == PlayerStatus.Idle || inputActive)
    ...
    {
        distanceToMax = this.transform.position.x - maxDepth;
        distanceToMin = minDepth - this.transform.position.x;
    }
    ...
    private void CalculateMovement()
    {
        ...
        if (playerStatus == PlayerStatus.MovinUp)
        {
            ...
            if (distanceToMin < depthEdge)
            {
                ...
                newSpeed = maxVerticalSpeed * (minDepth - this.transform.position.x) / depthEdge;
            ...
            {
                //Calculate maximum speed at this depth (without this, the sub would leave the gameplay are)
                newSpeed = maxVerticalSpeed * (maxDepth - this.transform.position.x) / depthEdge;
    ...
    private void MoveAndRotate()
    {
        ...
        //else
          //  _camera.transform.position += Vector3.right * speed * 0.6f * Time.deltaTime;
    }
    ...
    private void Sink()
    {
        ...
        float distance = this.transform.position.x - crashDepth;
        ...
        if (distance > 0.1f)
        {
            ...
            if (distance < crashDepthEdge)
            {
                ...
                newSpeed = maxVerticalSpeed * (crashDepth - this.transform.position.x) / crashDepthEdge;
        ...
        //{
            ...
            //Disable the smoke
           // StartCoroutine(FunctionLibrary.CallWithDelay(DisableSmoke, 0.5f));
       // }
    }
    ...
    private IEnumerator ReviveProcess()
    {
        ...
        newRotation = new Vector3(0, 0, 0);
        this.transform.eulerAngles = newRotation;
        StartCoroutine(FunctionLibrary.MoveElementBy(this.transform, new Vector2(0, Mathf.Abs(this.transform.position.x - maxDepth)), 0.5f));
